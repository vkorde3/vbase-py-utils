...
class TestRobustBetas(unittest.TestCase):
    ...
    def test_single_asset(self):
        ...
        beta_matrix = robust_betas(Y, X, half_life=30)
    ...
    def test_lambda_parameter(self):
        ...
        Y = pd.DataFrame({"Asset1": asset_returns})
        X = pd.DataFrame({"SPY": self.spy_returns})
        beta_matrix = robust_betas(Y, X, lambda_=0.985)
        self.assertAlmostEqual(
            beta_matrix.loc["SPY", "Asset1"], 1.5, delta=DEFAULT_DELTA
        )
    ...
    def test_invalid_half_life(self):
        ...
        for invalid_half_life in [0, -1]:
            with self.assertRaises(ValueError):
                robust_betas(Y, X, half_life=invalid_half_life)
    ...
    def test_invalid_lambda(self):
        ...
        for invalid_lambda in [0, 1.5]:
            with self.assertRaises(ValueError):
                robust_betas(Y, X, lambda_=invalid_lambda)
    ...
    def test_insufficient_timestamps(self):
        ...
        with self.assertRaises(ValueError):
            robust_betas(Y, X, half_life=2, min_timestamps=10)
